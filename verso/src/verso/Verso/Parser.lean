...
mutual
  ...
  where
    tok (count : Nat) : String := "".pushn char count
    ...
    normFn : ParserFn := fun _c s => Id.run <| do
      ...
      if let .atom info str := str then
        if str.startsWith "\" " && str.endsWith " \"" then
          let core := str.drop 2 |>.dropEnd 2
          if core.copy.any (· != ' ') then
            let str := "\"" ++ core ++ "\""
            let info : SourceInfo :=
              match info with
              | .none => .none
              | .synthetic start stop c => .synthetic (⟨start.byteIdx + 1⟩) (⟨stop.byteIdx - 1⟩) c
              | .original leading start trailing stop =>
                .original
                  { leading with stopPos := ⟨leading.stopPos.byteIdx + 1⟩ } ⟨start.byteIdx + 1⟩
                  { trailing with startPos := ⟨trailing.startPos.byteIdx - 1⟩ } ⟨stop.byteIdx - 1⟩
