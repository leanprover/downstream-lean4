...
/-- Dual intervals transport their elements through `toDual`.
`Icc (toDual a) (toDual b)` equals `(Icc b a).map toDual.toEmbedding`; see `Finset.Icc_toDual`. -/
...
/-- Dual intervals transport their elements through `toDual`.
`Iic (toDual a)` equals `(Ici a).map toDual.toEmbedding`; see `Finset.Iic_toDual`. -/
@[to_dual
/-- Dual intervals transport their elements through `toDual`.
`Ici (toDual a)` equals `(Iic a).map toDual.toEmbedding`; see `Finset.Ici_toDual`. -/]
