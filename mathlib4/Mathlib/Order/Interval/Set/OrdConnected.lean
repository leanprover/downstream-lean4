...
@[simp]
theorem ordConnected_dual {s : Set α} : OrdConnected (OrderDual.ofDual ⁻¹' s) ↔ OrdConnected s := by
  unsealing_newtype OrderDual =>
    exact ⟨fun h => by simpa only [ordConnected_def] using! h.dual, fun h => h.dual⟩
