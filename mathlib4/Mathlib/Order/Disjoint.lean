...
theorem Codisjoint.left_le_of_le_inf_right (h : a ⊓ b ≤ c) (hd : Codisjoint b c) : a ≤ c := by
  unsealing_newtype OrderDual =>
    exact @Disjoint.left_le_of_le_sup_right αᵒᵈ _ _ _ _ _ h hd.symm
...
theorem disjoint_toDual_iff [PartialOrder α] [OrderTop α] {a b : α} :
    Disjoint (toDual a) (toDual b) ↔ Codisjoint a b := by
  unsealing_newtype OrderDual =>
    exact Iff.rfl

@[to_dual (attr := simp, grind =)]
theorem disjoint_ofDual_iff [PartialOrder α] [OrderBot α] {a b : αᵒᵈ} :
    Disjoint (ofDual a) (ofDual b) ↔ Codisjoint a b := by
  unsealing_newtype OrderDual =>
    exact Iff.rfl
...
protected theorem codisjoint_iff [OrderTop α] [OrderTop β] {x y : α × β} :
  ...
  unsealing_newtype OrderDual =>
    exact @Prod.disjoint_iff αᵒᵈ βᵒᵈ _ _ _ _ _ _
