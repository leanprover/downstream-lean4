...
theorem tendsto_finset_Ici_atBot_atTop [Preorder α] [LocallyFiniteOrderTop α] :
  ...
  · refine tendsto_atTop.2 fun s ↦ ?_
    obtain ⟨a, ha⟩ := (directed_id (r := ((· ≥ ·) : α → α → Prop))).finset_le s
    exact (eventually_le_atBot a).mono fun b hb c hc ↦ by simpa using hb.trans (ha c hc)
