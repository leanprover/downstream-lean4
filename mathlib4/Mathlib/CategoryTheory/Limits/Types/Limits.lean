...
variable {F} in
@[deprecated limit.w_apply +typeChanged (since := "2026-02-17")]
theorem Limit.w_apply {j j' : J} {x : (limit F : Type u)} (f : j ⟶ j') :
    F.map f (limit.π F j x) = limit.π F j' x :=
  limit.w_apply _ _ _

@[deprecated limit.lift_π_apply +typeChanged (since := "2026-02-17")]
theorem Limit.lift_π_apply (s : Cone F) (j : J) (x : s.pt) :
    limit.π F j (limit.lift F s x) = s.π.app j x :=
  limit.lift_π_apply _ _ _

@[deprecated limMap_π_apply +typeChanged (since := "2026-02-17")]
