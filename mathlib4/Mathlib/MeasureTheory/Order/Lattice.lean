...
/-- `MeasurableSet[Mᵒᵈ] s` is by definition `MeasurableSet (⇑toDual ⁻¹' s)`, so both directions of
the order dualisation are measurable on the nose. -/
...
instance (priority := 100) OrderDual.instMeasurableSup₂ [Min M] [MeasurableInf₂ M] :
    MeasurableSup₂ Mᵒᵈ where
  measurable_sup := measurable_toDual.comp (measurable_inf.comp
    ((measurable_ofDual.comp measurable_fst).prodMk (measurable_ofDual.comp measurable_snd)))

instance (priority := 100) OrderDual.instMeasurableInf₂ [Max M] [MeasurableSup₂ M] :
    MeasurableInf₂ Mᵒᵈ where
  measurable_inf := measurable_toDual.comp (measurable_sup.comp
    ((measurable_ofDual.comp measurable_fst).prodMk (measurable_ofDual.comp measurable_snd)))
