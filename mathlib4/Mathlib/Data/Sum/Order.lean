...
def sumLexDualAntidistrib (α β : Type*) [LE α] [LE β] : (α ⊕ₗ β)ᵒᵈ ≃o βᵒᵈ ⊕ₗ αᵒᵈ where
  toEquiv := ofDual.trans (ofLex.trans ((Equiv.sumComm α β).trans
    ((Equiv.sumCongr toDual toDual).trans toLex)))
  map_rel_iff' := by
    unsealing_newtype OrderDual =>
      rintro (a | a) (b | b)
      · change
