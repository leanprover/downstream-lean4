...
theorem of_to_nat' : ∀ n : PosNum, Num.ofNat' (n : ℕ) = Num.pos n
  | 1 => by
      ...
      norm_cast
  | bit0 p => by
      simpa only [Nat.bit_false, Bool.cond_false, two_mul, of_to_nat' p]
        using! Num.ofNat'_bit false p
  | bit1 p => by
      simpa only [Nat.bit_true, Bool.cond_true, two_mul, of_to_nat' p] using! Num.ofNat'_bit true p
