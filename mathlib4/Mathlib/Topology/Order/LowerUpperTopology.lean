...
theorem isUpperSet_of_isOpen (h : IsOpen s) : IsUpperSet s := by
  unsealing_newtype OrderDual =>
    exact IsLower.isLowerSet_of_isOpen (α := αᵒᵈ) h
...
theorem tendsto_nhds_iff_not_le {β : Type*} {f : β → α} {l : Filter β} {x : α} :
    Filter.Tendsto f l (𝓝 x) ↔ ∀ y, ¬x ≤ y → ∀ᶠ z in l, ¬f z ≤ y := by
  unsealing_newtype OrderDual =>
    exact IsLower.tendsto_nhds_iff_not_le (α := αᵒᵈ)
...
-/
@[simp]
theorem closure_singleton (a : α) : closure {a} = Iic a := by
  unsealing_newtype OrderDual =>
    exact IsLower.closure_singleton (α := αᵒᵈ) _

protected theorem isTopologicalBasis : IsTopologicalBasis (upperBasis α) := by
  unsealing_newtype OrderDual =>
    exact IsLower.isTopologicalBasis (α := αᵒᵈ)
...
lemma isTopologicalBasis_insert_univ_subbasis :
    IsTopologicalBasis (insert univ {s : Set α | ∃ a, (Iic a)ᶜ = s}) := by
  unsealing_newtype OrderDual =>
    exact IsLower.isTopologicalBasis_insert_univ_subbasis (α := αᵒᵈ)
...
lemma isTopologicalSpace_basis (U : Set α) : IsOpen U ↔ U = univ ∨ ∃ a, (Iic a)ᶜ = U := by
  unsealing_newtype OrderDual =>
    exact IsLower.isTopologicalSpace_basis (α := αᵒᵈ) U
...
protected lemma _root_.sSupHom.continuous (f : sSupHom α β) : Continuous f := by
  unsealing_newtype OrderDual =>
    exact sInfHom.continuous (α := αᵒᵈ) (β := βᵒᵈ) (sSupHom.dual.toFun f)
