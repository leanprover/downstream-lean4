...
theorem dite_eq_iff' : dite P A B = c ↔ (∀ h, A h = c) ∧ ∀ h, B h = c :=
  ⟨fun he ↦ ⟨fun h ↦ (dite_eq_left h).symm.trans he, fun h ↦ (dite_eq_right h).symm.trans he⟩, fun he ↦
    (em P).elim (fun h ↦ (dite_eq_left h).trans <| he.1 h) fun h ↦ (dite_eq_right h).trans <| he.2 h⟩
